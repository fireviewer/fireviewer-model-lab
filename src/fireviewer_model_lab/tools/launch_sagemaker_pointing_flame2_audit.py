"""Build the SageMaker request for FLAME2 pointing-source admission audit."""
...
DEFAULT_SOURCE = (
    "pointing-corpus-v2/new-sources/robofirefusenet-flame2/0d8ec502da0bafea7c388a989650aa53d1ecf278"
)


def build_request(args: argparse.Namespace, job_name: str) -> dict:
    values = (args.source_prefix, args.baseline_prefix, args.code_prefix, args.output_prefix)
    ...
    if any(marker in value.lower() for value in values for marker in forbidden):
        raise ValueError("FLAME2 audit escaped the isolated pointing campaign")
    bucket = args.bucket
    source_uri = f"s3://{bucket}/{args.source_prefix.rstrip('/')}"
    return {
        ...
        "AppSpecification": {
            ...
            "ContainerArguments": [
                "/opt/ml/processing/input/code/pointing_flame2_audit.py",
                "--source-root",
                "/opt/ml/processing/input/flame2",
                "--baseline-root",
        ...
        "ProcessingInputs": [
            ...
            },
            {
                ...
                "S3Input": {
                    "S3Uri": source_uri,
                    "LocalPath": "/opt/ml/processing/input/flame2",
                    "S3DataType": "S3Prefix",
                    "S3InputMode": "File",
                    "S3DataDistributionType": "FullyReplicated",
                    "S3CompressionType": "None",
            ...
            },
            {
                "InputName": "baseline",
                "S3Input": {
                    "S3Uri": f"s3://{bucket}/{args.baseline_prefix.rstrip('/')}",
        ...
        "Tags": [
            ...
            {"Key": "fireviewer:stage", "Value": "flame2-admission-audit"},
            {
                "Key": "fireviewer:point-derivation",
                "Value": "deterministic-source-mask-only",
            },
        ],
    }
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--source-prefix", default=DEFAULT_SOURCE)
    parser.add_argument("--baseline-prefix", required=True)
    parser.add_argument("--code-prefix", default="pointing-corpus-v2/code/flame2-audit")
    parser.add_argument("--output-prefix", default="pointing-corpus-v2/reports/flame2-audit")
...
def main() -> None:
    ...
    job_name = args.job_name or f"fireviewer-pointing-flame2-audit-{stamp}"
