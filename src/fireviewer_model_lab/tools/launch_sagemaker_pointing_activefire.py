"""Build the SageMaker request for strict ActiveFire pointing admission."""
...
def build_request(args: argparse.Namespace, job_name: str) -> dict:
    values = (args.code_prefix, args.baseline_prefix, args.output_prefix)
    ...
    if any(marker in value.lower() for value in values for marker in forbidden):
        raise ValueError("ActiveFire audit escaped the isolated pointing campaign")
    bucket = args.bucket
    ...
    return {
        ...
        "AppSpecification": {
            ...
            "ContainerArguments": [
                "/opt/ml/processing/input/code/pointing_activefire_bootstrap.py",
                "--baseline-root",
        ...
        "Tags": [
            ...
            {"Key": "fireviewer:stage", "Value": "activefire-strict-admission"},
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--code-prefix", default="pointing-corpus-v2/code/activefire-audit")
...
def main() -> None:
    ...
    job_name = args.job_name or f"fireviewer-pointing-activefire-{stamp}"
