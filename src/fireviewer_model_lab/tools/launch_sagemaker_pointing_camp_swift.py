"""Build the SageMaker request for strict Camp Swift pointing admission."""
...
def build_request(args: argparse.Namespace, job_name: str) -> dict:
    prefixes = (args.source_prefix, args.baseline_prefix, args.code_prefix, args.output_prefix)
    ...
    if any(marker in value.lower() for value in prefixes for marker in forbidden):
        raise ValueError("Camp Swift audit escaped the isolated pointing campaign")
    bucket = args.bucket
    ...
    return {
        ...
        "AppSpecification": {
            ...
            "ContainerArguments": [
                "/opt/ml/processing/input/code/pointing_camp_swift_audit.py",
                "--source-root",
                "/opt/ml/processing/input/source",
                "--baseline-root",
        ...
        "ProcessingInputs": [
            ...
            _input(
                "camp-swift-source",
                f"s3://{bucket}/{args.source_prefix.rstrip('/')}",
                "/opt/ml/processing/input/source",
            ),
            _input(
                "strict-baseline",
        ...
        "Tags": [
            ...
            {"Key": "fireviewer:stage", "Value": "camp-swift-strict-admission"},
...
def main() -> None:
    ...
    job_name = args.job_name or f"fireviewer-pointing-camp-swift-{stamp}"
