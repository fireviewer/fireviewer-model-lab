"""Build a SageMaker Processing request for the full pointing pixel audit."""
...
def build_request(args: argparse.Namespace, job_name: str) -> dict:
    values = (args.stage1_prefix, args.code_prefix, args.output_prefix)
    ...
    if any(marker in value.lower() for value in values for marker in forbidden):
        raise ValueError("pixel audit input is outside the isolated pointing campaign")
    bucket = args.bucket
    return {
        ...
        "AppSpecification": {
            ...
            "ContainerArguments": [
                "/opt/ml/processing/input/code/pointing_pixel_audit.py",
                "--input-dir",
                "/opt/ml/processing/input/stage1",
                "--output-dir",
        ...
        "ProcessingInputs": [
            ...
            },
            {
                "InputName": "stage1",
                "S3Input": {
                    "S3Uri": f"s3://{bucket}/{args.stage1_prefix.rstrip('/')}",
                    "LocalPath": "/opt/ml/processing/input/stage1",
                    "S3DataType": "S3Prefix",
                    "S3InputMode": "File",
                    "S3DataDistributionType": "FullyReplicated",
                    "S3CompressionType": "None",
        ...
        "Tags": [
            ...
            {"Key": "fireviewer:stage", "Value": "full-pixel-audit"},
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--code-prefix", default="pointing-corpus-v2/code/stage2")
    parser.add_argument("--output-prefix", default="pointing-corpus-v2/reports/stage2")
...
def main() -> None:
    ...
    job_name = args.job_name or f"fireviewer-pointing-v2-pixels-{stamp}"
