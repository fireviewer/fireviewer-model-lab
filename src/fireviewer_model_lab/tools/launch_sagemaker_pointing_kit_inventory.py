"""Build a SageMaker request for the KIT Industrial Burner Flames inventory."""
...
def build_request(args: argparse.Namespace, job_name: str) -> dict:
    ...
    if any(marker in value.casefold() for value in values for marker in forbidden):
        raise ValueError("KIT inventory escaped the isolated pointing campaign")
    bucket = args.bucket
    return {
        ...
        "AppSpecification": {
            ...
            "ContainerArguments": [
                "/opt/ml/processing/input/code/pointing_kit_inventory.py",
                "--output-dir",
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--code-prefix", default="pointing-corpus-v2/code/kit-inventory")
    parser.add_argument(
        "--output-prefix",
        default="pointing-corpus-v2/reports/kit-inventory",
    )
...
def main() -> None:
    ...
    job_name = args.job_name or f"fireviewer-pointing-kit-inventory-{stamp}"
