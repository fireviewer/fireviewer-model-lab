# Organisation de fireviewer-model-lab

Source canonique : **[fireviewer/fireviewer-model-lab](https://github.com/fireviewer/fireviewer-model-lab)**. Responsabilité technique : **FV**. Accès : **private**.

Préparation des corpus, entraînement, benchmarks et registre des modèles.

## Où travailler et commiter

Dans le workspace organisé, ouvrir **`actifs/fireviewer/fireviewer-model-lab`**. Chaque dossier est un dépôt Git autonome. Créer une branche de chantier dans ce dépôt ; pour un travail parallèle, créer un worktree sous `travaux/<chantier>/<composant>`. Ne jamais commiter depuis la racine multi-dépôts, `archives/` ou un ancien chemin de compatibilité.

Recettes et registre canonique ; fonctions synthétiques indépendantes conservées.

Poids et datasets hors Git. Les anciennes chaînes Blender/Omniverse et le pack Hunyuan3D/Asset4Sim sont historiques.
